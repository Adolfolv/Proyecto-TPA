...
class ConstructorHistorial:
    ...
    def _crear_cabecera(self, padre):
        ...
        self.moldes.crear_label(cabecera, "Historial de viajes", tema.FUENTE_TITULO, tema.TEXTO, tema.PANEL, metodo="grid", fila=0, columna=0, sticky="w")
    ...
    def _crear_contenido(self, padre):
        vista = self.vista
        panel = self.moldes.crear_frame(padre, tema.PANEL, fila=1, columna=1, sticky="nsew", columnas_peso=((0, 1),), filas_peso=((1, 3), (4, 2)))
        ...
        vista.tabla_viajes = self.moldes.crear_tabla(panel, (("fecha", "Fecha", 110), ("ruta", "Ruta", 200), ("modalidad", "Procedencia", 95), ("tipo", "Tipo", 70), ("monto", "Monto", 75)), alto=7, metodo="grid", fila=1, columna=0, sticky="nsew")
        vista.tabla_viajes.bind("<<TreeviewSelect>>", vista.acciones.mostrar_seleccion)
        vista.label_sin_viajes = self.moldes.crear_label(panel, "", tema.FUENTE_TEXTO, tema.TEXTO_SUAVE, tema.PANEL, metodo="grid", fila=2, columna=0, sticky="w", margen_y=(5, 10))

        cabecera = self.moldes.crear_frame(panel, tema.PANEL, fila=3, columna=0, sticky="ew", margen_y=(6, 8), columnas_peso=((0, 1),))
        ...
        vista.grafico_actividad = tk.Canvas(panel, bg=tema.SECUNDARIO, highlightbackground=tema.BORDE, highlightthickness=1, height=190)
        vista.grafico_actividad.grid(row=4, column=0, sticky="nsew")
        vista.grafico_actividad.bind("<Configure>", vista.renderizador.dibujar_actividad)


class AccionesHistorial:
    """Coordina consultas, filtros y selección de filas."""
...
class RenderizadorHistorial:
    ...
    def renderizar_viajes(self, viajes):
        ...
        vista.label_detalle.config(text="Selecciona un viaje de la tabla.")

    def renderizar_actividad(self, resumen):
        vista = self.vista
        vista.actividad = resumen.actividad
        signo = "↑" if resumen.tendencia_porcentual > 0 else "↓" if resumen.tendencia_porcentual < 0 else "="
        vista.label_tendencia.config(text=f"{resumen.total_ultimos_7_dias} viajes · {signo} {abs(resumen.tendencia_porcentual):.1f}%")
