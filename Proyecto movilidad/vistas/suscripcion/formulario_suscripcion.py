...
class FormularioSuscripcion:
    ...
    def __init__(self, padre, moldes, lugares, accion_previsualizar):
        ...
        self.variables_dias = []
        self.checks_dias = []
        self.panel = self.moldes.crear_frame(padre, tema.PANEL_SUAVE, tema.BORDE, 1, 18, 18, fila=0, columna=0, sticky="nsew", columnas_peso=((0, 1), (1, 1)))
        self.moldes.crear_label(self.panel, "Nueva suscripcion", tema.FUENTE_SUBTITULO, tema.TEXTO, tema.PANEL_SUAVE, metodo="grid", fila=0, columna=0, columnas=2, sticky="w", margen_y=(0, 10))
        ...
        self._crear_dias(7)
        self.boton_crear = self.moldes.crear_boton(self.panel, "Crear suscripcion", True, None, accion_previsualizar, metodo="grid", fila=9, columna=0, columnas=2, sticky="ew", margen_y=(14, 0))
    ...
    def habilitar(self, habilitado=True):
        ...
        self.boton_crear.configure(state=estado_entrada, cursor="hand2" if habilitado else "arrow")
    ...
    def datos(self):
        return {
            ...
            "dias_semana": tuple(indice for indice, variable in enumerate(self.variables_dias) if variable.get()),
            "hora": self.hora.get(),
            "cantidad_pasajeros": self.pasajeros.get(),
        }

    def _crear_etiqueta(self, texto, fila, columna):
        self.moldes.crear_label(self.panel, texto, tema.FUENTE_BOTON, tema.TEXTO_SUAVE, tema.PANEL_SUAVE, metodo="grid", fila=fila, columna=columna, sticky="w", margen_x=5, margen_y=(5, 3))

    def _crear_selector(self, titulo, opciones, fila, columna):
        self._crear_etiqueta(titulo, fila, columna)
        return self.moldes.crear_selector(self.panel, opciones, metodo="grid", fila=fila + 1, columna=columna, sticky="ew", margen_x=5, margen_y=(0, 5))

    def _crear_entrada(self, titulo, fila, columna, valor):
        self._crear_etiqueta(titulo, fila, columna)
        entrada = self.moldes.crear_entrada(self.panel, metodo="grid", fila=fila + 1, columna=columna, sticky="ew", margen_x=5, margen_y=(0, 5))
        entrada.insert(0, valor)
        return entrada

    def _crear_dias(self, fila):
        self.moldes.crear_label(self.panel, "Dias de la semana", tema.FUENTE_BOTON, tema.TEXTO_SUAVE, tema.PANEL_SUAVE, metodo="grid", fila=fila, columna=0, columnas=2, sticky="w", margen_x=5, margen_y=(8, 3))
        contenedor = self.moldes.crear_frame(self.panel, tema.PANEL_SUAVE, fila=fila + 1, columna=0, columnas=2, sticky="ew")
