...
from .estado_visual_suscripcion import EstadoConfirmacionPago, EstadoCotizacion, EstadoFormulario, EstadoProcesandoPago, FlujoVisualSuscripcion
...
class VistaSuscripcionViaje(tk.Frame):
    ...
    def __init__(self, padre, navegar, controlador, usuario_actual, controlador_viaje):
        ...
        if getattr(self.usuario_actual, "tipo_usuario", "") != "pasajero":
            ...
            self.mostrar_mensaje("Solo las cuentas de pasajero pueden crear suscripciones.")
    ...
    def _crear_widgets(self, lugares):
        ...
        creacion = self.moldes.crear_frame(cuerpo, tema.PANEL, fila=0, columna=0, sticky="nsew", margen_x=(0, 8), columnas_peso=((0, 1),), filas_peso=((0, 1),))
        self.formulario = FormularioSuscripcion(creacion, self.moldes, lugares, self.previsualizar_suscripcion)
        self.tarjeta = TarjetaResumenSuscripcion(creacion, self.moldes, {"editar": self.editar_datos, "pagar": self.solicitar_confirmacion_pago, "confirmar": self.confirmar_pago, "cancelar": self.cancelar_alta})
        self.gestion = PanelGestionSuscripciones(cuerpo, self.moldes, {"alternar": self.alternar_suscripcion, "cancelar_suscripcion": self.cancelar_suscripcion, "cancelar_viaje": self.cancelar_viaje})
    ...
    def previsualizar_suscripcion(self):
        ...
        self.mostrar_mensaje("Cotizacion generada. Revisa los datos antes de pagar.", True)
    ...
    def solicitar_confirmacion_pago(self):
        ...
        self.mostrar_mensaje("Confirma el pago para activar y guardar la suscripcion.", True)
    ...
    def confirmar_pago(self):
        ...
        resultado = self.controlador.confirmar(self.usuario_actual, self.datos_pendientes)
        if not resultado.exitoso:
            self.flujo.cambiar(EstadoConfirmacionPago)
        ...
        self.flujo.cambiar(EstadoFormulario)
        self.actualizar_listados()
        self.mostrar_mensaje("Pago realizado. La suscripcion ya aparece en tus listados.", True)

    def cancelar_alta(self):
        self.datos_pendientes = None
        self.resumen_pendiente = None
        self.flujo.cambiar(EstadoFormulario)
        self.mostrar_mensaje("Suscripcion cancelada antes del pago. No se realizo ningun cobro.", True)
    ...
    def cancelar_suscripcion(self):
        ...
        if suscripcion is None:
            self.mostrar_mensaje("Revisa este dato: selecciona una suscripcion.")
            return
        if not messagebox.askyesno("Cancelar suscripcion", "Se cancelaran y reembolsaran sus viajes pendientes. Continuar?"):
            return
        resultado = self.controlador.cambiar_estado(self.usuario_actual, suscripcion.id_suscripcion, "CANCELADA")
        self._finalizar_accion(resultado, "Suscripcion cancelada y viajes pendientes reembolsados.")
    ...
    def cancelar_viaje(self):
        ...
        self._finalizar_accion(resultado, "Viaje cancelado y monto reembolsado.")
    ...
    def actualizar_listados(self):
        ...
        if not resultado_suscripciones.exitoso or not resultado_viajes.exitoso:
            self.mostrar_mensaje(resultado_suscripciones.error or resultado_viajes.error)
            return
